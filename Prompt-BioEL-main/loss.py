...
class MultiLabelLoss(nn.Module):
    ...
    def forward(self, logits, label_marks):
        ...
        elif self.type_loss == 'bce_loss':
            return bce_loss(logits,label_marks,self.reduction)
...
def log_sum_loss(logits, mask, reduction='sum'):
    ...
    gold_logits = logits.masked_fill(~(mask.bool()), -10000)  # 如果不是
...
def bce_loss(logits, mask, reduction="sum"):
    # gold_scores = torch.log(logits).masked_fill(~(mask.bool()),0).sum(-1)
    # neg_scores = torch.log(1-logits).masked_fill(mask.bool(),0).sum(-1)
    # loss = -( gold_scores + neg_scores)
    # loss = loss.sum(-1)
    loss_fnc = nn.BCELoss()
    loss = loss_fnc(logits,mask)
